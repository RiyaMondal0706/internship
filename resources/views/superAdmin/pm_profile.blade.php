            ...
            <div class="row g-0">
                <div class="col-lg-4 bg-light border-end p-5 text-center">
                    <div class="mb-4 position-relative d-inline-block">
                        <img src="{{ asset('images/Projectmanager/' . $hr->image) }}"
                    ...
                    <h4 class="fw-bold text-dark mb-1">{{ $hr->name }} </h4>
                    <p class="text-muted small mb-4">{{ strtoupper($hr->designation) }} • ID: #HR-9921</p>

                    <div class="d-grid gap-2">
                        <a href="https://mail.google.com/mail/?view=cm&fs=1&to={{ $hr->email }}&su=Hello&body=Dear {{ $hr->name }},"
                            target="_blank" class="btn btn-primary shadow-sm">
                            <i class="bi bi-envelope me-2"></i>Send Email
                        </a>
                        <a href="{{ route('pm.edit', $hr->id) }}" class="btn btn-outline-dark">
                    ...
                    <div class="row g-4">
                        <div class="col-sm-6">
                            <div class="p-3 border rounded-3 bg-white shadow-sm">
                                ...
                                <div class="fw-bold text-dark">{{ $hr->email }}</div>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="p-3 border rounded-3 bg-white shadow-sm">
                                <label class="d-block text-muted small mb-1">Mobile Number</label>
                                <div class="fw-bold text-dark">+91 {{ $hr->phone }}</div>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="p-3 border rounded-3 bg-white shadow-sm">
                                ...
                                <div class="fw-bold text-dark">{{ strtoupper($hr->designation) }}</div>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="p-3 border rounded-3 bg-white shadow-sm">
                                ...
                                <div class="fw-bold text-dark">
                                    {{ \Carbon\Carbon::parse($hr->joining)->format('d F Y') }}</div>
                            </div>
                        </div>
                            ...
                            <div class="p-4 border rounded-3 bg-light-subtle">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <span class="fw-medium">{{ $hr->address }}</span>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="text-muted small d-block">City</label>
                                        <span class="fw-medium">{{ $hr->city }}</span>
                                    </div>
                                    @php
                                        $state = DB::table('states')->where('id', $hr->state)->first();
                                    ...
                                    @php
                                        $districts = DB::table('districts')->where('id', $hr->district)->first();
