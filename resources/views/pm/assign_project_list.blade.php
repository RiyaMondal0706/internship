                    ...
                    <tbody>
                        @foreach ($ass as $item)
                            <tr>
                                ...
                                <td>{{ $emp->name ?? 'Not Assigned' }}</td>

                                <td>
                                    @if ($item->designation == 'teamlead')
                                        Team Leader
                                    @elseif($item->designation == 'employee')
                                        Employee
                                    @elseif($item->designation == 'intern')
                                        Intern
                                    @else
                                        {{ $item->designation }}
                                    @endif
                                ...
                                <td>{{ \Illuminate\Support\Str::limit($item->work, 15) }}</td>

                                <td>
                                    <a href="{{ route('pm.assign.project.status', $item->id) }}">
                                        @if ($item->status == 1)
                                ...
                                <td>
                                    <a href="javascript:void(0)" onclick="archiveProject({{ $item->id }})">
        ...
        function archiveProject(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    window.location.href = "/Project-Manager/project/archive/" + id;
